...
tail -n +2 "../sourcedata/mpm_cuprizone_bruker_scan_directories.csv" | while IFS=',' read -r sourcedata_folder subject_id session_id structural_scanidx flip1_TB1DAM_scanidx flip2_TB1DAM_scanidx mpm_pdw_scanidx mpm_mtw_scanidx mpm_t1w_scanidx; do
    ...
    if [[ $flip1_TB1DAM_receivergain==$flip2_TB1DAM_receivergain ]]; then
        ...
        printf "%f\n" $flip1_TB1DAM_receivergain $flip2_TB1DAM_receivergain $mpm_pdw_receivergain $mpm_mtw_receivergain $mpm_t1w_receivergain
    fi

    # Check if all receiver gains are the same for MPM scans
    if [[ $mpm_pdw_receivergain==$mpm_mtw_scanidx ]] && [[ $mpm_pdw_receivergain==$mpm_t1w_scanidx ]]; then
        printf "All receiver gains are the same value for MPM scans: $mpm_pdw_receivergain\n\n"
    else
        printf "\n\n\nWARNING: Reference powers are different within same session\n"
        printf "%f\n" $mpm_pdw_receivergain $mpm_mtw_scanidx $mpm_t1w_scanidx
    fi
