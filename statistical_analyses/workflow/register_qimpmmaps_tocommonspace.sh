...
# ARG_HELP([Use twolevel_commonspace_resample.sh to register qi MPM maps to common space])
# ARGBASH_GO()
...
print_help()
{
	printf '%s\n' "Use twolevel_commonspace_resample.sh to register qi MPM maps to common space"
...
# Define common path
twolevel_modelbuid_outputdir=../derivatives/registration/modelbuild/
...
for i_file_to_register in ${file_to_register[@]}
  ...
  twolevel_commonspace_resample.sh ${twolevel_modelbuid_outputdir}inputs.txt \
    --output-dir ${twolevel_modelbuid_outputdir} \
...
qbatch --depend commonspace_resample* --logdir ${_arg_registration_output_dir}/log -- \
  rsync -avz --delete --remove-source-files \
    ../derivatives/registration/modelbuild/firstlevel/commonspace-resampled/ \
