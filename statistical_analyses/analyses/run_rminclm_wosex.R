...
dir.create("../derivatives/statistical_analyses/rminc_outputs/wosex/", showWarnings = FALSE)
...
for (i_mpm_map in mpm_maps) {
   ...
   model <- mincLm(Filename ~ Treatment, csv_file_content_df, mask=brain_mask)
   print(model)
   thresholds = attr(mincFDR(model, mask=brain_mask), "thresholds")
   print(thresholds)

   mincWriteVolume(model, paste0("../derivatives/statistical_analyses/rminc_outputs/wosex/tvalue-TreatmentD-AIP_",i_mpm_map,".mnc"), "tvalue-TreatmentD-AIP")
   mincWriteVolume(model, paste0("../derivatives/statistical_analyses/rminc_outputs/wosex/tvalue-TreatmentS-AIP_",i_mpm_map,".mnc"), "tvalue-TreatmentS-AIP")

   for (predictor in dimnames(thresholds)[[2]][c(-1,-2)]) {
 
      #If you want to save this to a file, uncomment next line
      svg(paste0("../derivatives/statistical_analyses/rminc_outputs/wosex/pbs_vs_",predictor,"_",i_mpm_map,"_tstatfdrthresh-p2.svg"), height = 3.1, width = 4)
